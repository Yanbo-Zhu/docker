...
## 6.5 docker system df 查看镜像/容器/数据卷所占的空间

![](image/Pasted%20image%2020240207164710.png)

## 6.6 docker rmi 某个XXX镜像名字ID  删除
...
例子: docker build -f Dockerfile-el8 -t rpm-builder-8 . 
...
### 7.4.4 --volumes-from  卷的继承

docker run -it  --privileged=true --volumes-from 父类  --name u2 ubuntu

## 7.5 docker exec / attach 进入正在运行的容器并以命令行交互
1 
...
## 7.10 docker commit 从 某个 container 产生 新的 image  
...
## 7.11 docker export import 6.8 导入和导出容器
