...
## 2.3 Linux 容器 / Linux container/ ·容器虚拟化技术
...
Docker是C/S（客户端client-服务器server）架构模式。Docker通过客户端连接守护进程，通过命令向守护进程发出请求，守护进程通过一系列的操作返回结果。docker客户端和服务器通过socket或RESTful API进行通信。
